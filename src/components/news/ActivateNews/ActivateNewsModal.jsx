...
import { ActiveNews } from "../../../core/services/api/put/News/ActiveNews";

const SelectSchema = Yup.object().shape({
  active: Yup.string().required("یک گزینه انتخاب کنید"),
});
const ActivateNewsModal = ({ activeData, toggleActiveModal, isOpen }) => {
  const queryClient = useQueryClient();
  const { mutate: changeActive } = useMutation({
    mutationKey: ["ACTIVENEWS"],
    mutationFn: (payload) => ActiveNews(payload),
    onError: (err) => {
      if (err) {
...
export default ActivateNewsModal;
