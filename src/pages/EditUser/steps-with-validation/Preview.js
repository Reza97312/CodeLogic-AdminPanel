...
import { UpdateUser } from "../../../core/services/api/put/UpdateUser";
import { toast } from "react-toastify";
import { useNavigate } from "react-router-dom";

const Preview = ({ stepper, initialData, payload }) => {
  const navigate = useNavigate();
  const { mutate: updateU, isLoading: isPending } = useMutation({
    mutationKey: ["UPDATEUSER"],
    mutationFn: (bod) => UpdateUser(bod),
    onSuccess: (data) => {
      ...
      <div className="d-flex align-items-center justify-content-center mb-3 text-center">
        ...
        <div className="flex-grow-1 ">
          ...
          {children ? (
            ...
            <span className="d-block font-medium-2 text-dark">
              {value || "ثبت نشده"}
            </span>
          )}
        </div>
  ...
  return (
    <Fragment>
        ...
        <Row>
          ...
          <Col md={6} className="text-center">
            ...
            <InfoItem
              ...
              isLink
            />
            <InfoItem icon={Link} label="تلگرام" value={telegramLink} isLink />
      ...
      <div className="d-flex justify-content-between mt-5 pt-3">
        ...
        <Button
          onClick={() => updateU(payload)}
          color="success"
          className="btn-next"
        >
          <span className="align-middle d-sm-inline-block d-none">
            {isPending ? "درحال ارسال" : "ارسال اطلاعات"}
          </span>
